...
def solve_fifo(instance):
    ...
    sorted_ships = sorted(ships, key=lambda s: (s.arrival_time, s.id))
...
def solve_greedy(instance):
    ...
    sorted_ships = sorted(
        ships,
        key=lambda s: (min(s.cost_shore, s.cost_battery, s.cost_brown), s.arrival_time),
    )
    for s in sorted_ships:
        can_shore = False
        dur_shore = max(s.t_cargo, s.t_shore_power)
